...
### 人机验证对抗（打码平台，2026-08-05 接入）

发码偶发触发滑块/点选验证（小红书自研盾 burdock，`fe_api/burdock/v2/shield`，
非极验——公开打码类型不一定认）。方案：`collectors/captcha_solver.py` 封装
[超级鹰](https://www.chaojiying.com)打码平台，验证图片上传后人工打码兜底返回
坐标，本地模拟人类拖动轨迹（先快后慢 + 抖动）完成验证。

- 环境变量（.bashrc 已设占位，注册超级鹰后填入）：
  - `CAPTCHA_USER`：超级鹰用户名
  - `CAPTCHA_PASS`：密码明文（仅内存中 md5 后传参，不落盘）
  - `CAPTCHA_SOFTID`：软件ID（用户中心生成，可空）
- 类型：滑块拼图 9602（返回两坐标，缺口水平距离 = |x1-x2|）、点选 9004
  （1~4 坐标）；价格 36 题分/次 ≈ 0.036 元，识别错误 3 分钟内可返分
  （`report_error`，拖动后弹窗未消失自动返分）
- **未配置凭证时遇滑块会飞书提示"请手动完成验证"**，不影响无滑块时的正常流程
- 限制与待校准：自研盾行为检测可能识破轨迹模拟（拖动后弹窗不消失即判定失败，
  宁可报错不盲试）；滑块弹窗 DOM 未实测过（按文本特征"向右滑动/拖动滑块/滑块
  填充"检测），真实遇到时用截图校准弹窗容器与滑钮选择器
